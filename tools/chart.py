...
@tool
def render_pie_chart(title: str, labels: list[str], values: list[float], filename: str):
    ...
    if len(labels) != len(values):
        return "Error: labbls anddvalues must be the same length."
    ...
    return f"Save to outputs/{safe_name}"